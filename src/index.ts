...
function determineWinner(state: { team_a_score: number; team_b_score: number; team_a_name: string; team_b_name: string }): string | null {
    ...
    } else if (state.team_b_score > state.team_b_score) {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        ...
        if (request.method === 'POST') {
         ...
         if (request.method === 'GET' && pathParts.length === 4) { // Ensure it's just /api/tournament_matches/{id}
             ...
             try {
                 ...
                 if (updates.team1_id !== undefined && updates.team2_id !== undefined && updates.team1_id === updates.team2_id) {
                      return new Response(JSON.stringify({ error: "队伍A和队伍B不能是同一支队伍。" }), { status: 400, headers: { 'Content-Type': 'application/json' } });
                 }
    ...
    // --- Bulk Import Tournament Matches Endpoint ---
     if (url.pathname === '/api/tournament_matches/bulk' && request.method === 'POST') {
        try {
            const matchesData = await request.json<Omit<TournamentMatch, 'id' | 'match_do_id' | 'status' | 'winner_team_id' | 'created_at' | 'team1_code' | 'team1_name' | 'team2_code' | 'team2_name' | 'winner_team_code' | 'winner_team_name'>[]>();
            ...
            for (const match of matchesData) {
                 if (!match.tournament_round || match.match_number_in_round === undefined || isNaN(Number(match.match_number_in_round)) || Number(match.match_number_in_round) < 1 || match.team1_id === undefined || isNaN(Number(match.team1_id)) || match.team2_id === undefined || isNaN(Number(match.team2_id))) {
                      return new Response(JSON.stringify({ error: `Invalid match data found in input: round='${match.tournament_round}', number='${match.match_number_in_round}', team1_id='${match.team1_id}', team2_id='${match.team2_id}'. Round, number (>=1), team1_id, team2_id are required and must be valid.` }), { status: 400, headers: { 'Content-Type': 'application/json' } });
                 }
                 if (Number(match.team1_id) === Number(match.team2_id)) {
                 ...
                 if (roundMatchNumbers.has(key)) {
                      return new Response(JSON.stringify({ error: `Duplicate match found in input data: Round '${match.tournament_round}', Number ${match.match_number_in_round}.` }), { status: 400, headers: { 'Content-Type': 'application/json' } });
                 }
                 roundMatchNumbers.add(key);

                 teamIds.add(Number(match.team1_id));
                 teamIds.add(Number(match.team2_id));
            }
            ...
            const insertBatch = matchesData.map(match => {
                 return stmt.bind(
                     ...
                     Number(match.match_number_in_round), // Ensure number type
                     Number(match.team1_id), // Ensure number type
                     Number(match.team2_id), // Ensure number type
            ...
            console.error("Worker exception during bulk create tournament matches:", e);
    ...
    return new Response('Not found. API endpoints are /api/match/*, /api/teams, /api/teams/:id, /api/teams/bulk, /api/members, /api/members/:id, /api/teams/:team_code/members, /api/members/bulk, /api/tournament_matches, /api/tournament_matches/:id, /api/tournament_matches/bulk, /api/tournament_matches/:id/start_live, /api/archived_rounds, /api/archived_rounds/:id (PUT), /api/archived_matches', { status: 404 });
  },
};
